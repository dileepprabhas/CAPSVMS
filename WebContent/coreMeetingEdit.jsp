...
// int n =st.executeUpdate("insert into question1 select subjectid from subject where subjectName='"+subjectid+" && insert into question1 values ('"+questionid+"','"+content+"')");
 ...
 st = connection.prepareStatement("update  sub_admin_minutes_meeting set  venue=?, meeting_called_by= ?,facilitator = ?,note_taken =?, topics=?,action= ? , discussion=?, status = ? where id='"+id+"'"); 
